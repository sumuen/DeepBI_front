...
const Dialogue = props => {
  ...
  const sendSocketMessage = useCallback(
    (state, sender, data_type, content, id = 0) => {
      const messgaeInfo = {
        state,
  ...
  const onSuccess = useCallback(
    (code, value, source_item, result, firstTableData) => {
      if (!lockReconnect) {
      ...
      try {
        ...
        if (data.receiver === "bi") {
          if (data.data.data_type === "mysql_code") {
            ...
            if (chat_type === "autopilot") {
              setState({
                messages: [
                  ...
                  { content: "", sender: "user", time: moment().format("YYYY-MM-DD HH:mm") },
                ],
              });
            } else {
              setState({
            ...
            setLoadingState(false);
            toast.success(
              window.W_L.configuration_completed + " " + chat_type === "autopilot" ? "" : window.W_L.start_the_dialogue,
              {
                icon: "👏",
              }
            );
  ...
  const closeSetMessage = () => {
    if (chat_type === "chat" || chat_type === "report") {
      ...
      if (allMessages.length > 0) addChatList(allMessages, chat_type);
    }
  };
  useEffect(() => {
  ...
  const isSendTableDate = useCallback(
    data_type => {
      handleSocketMessage();
      if (!lockReconnect) {
  ...
  const handleSendMessage1 = useCallback(async () => {
    ...
    setState(prevState => ({
      ...
      messages: [
        ...messages,
        { content: inputMessage, sender: "user", time: moment().format("YYYY-MM-DD HH:mm") },
        { content: "", sender: "bot", Cardloading: true },
      ],
      inputMessage: "",
      data_type: "question",
    }));
  ...
  const setData_type = useCallback(
    value => {
  ...
  const retry = useCallback(index => {}, [setState, sendSocketMessage]);
  ...
  const { saveChart, dashboardsId, publishQuery } = useChartCode(
    sendSocketMessage,
    saveDashboardId,
    props,
    successSetting,
    CharttableD_date.current,
    new_sql,
    dashboardId,
    sendDashId
  );
  ...
  return (
    <div className="dialogue-content">
      <DialogueTop
        ...
        chat_type={chat_type}></DialogueTop>
